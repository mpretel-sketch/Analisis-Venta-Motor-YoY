...
def build_pdf_report_bytes(result: Dict) -> bytes:
    from io import BytesIO
    import os
    ...
    logo_path = os.getenv("PDF_LOGO_PATH", "/Users/miguelpretelpozo/Desktop/logo.png")

    # Header with branding
    c.setFillColor(HexColor(primary))
    c.setFont("Helvetica-Bold", 16)
    c.drawString(40, height - 50, "Executive YoY BE Control")
    ...
    try:
        if logo_path and os.path.exists(logo_path):
            c.drawImage(ImageReader(logo_path), width - 140, height - 80, width=90, height=40, preserveAspectRatio=True, mask='auto')
    except Exception:
        pass

    # KPI cards
    kpis = [
        ("Facturación año anterior", result["summary"]["totalPrev"]),
        ("Facturación año actual", result["summary"]["totalCurr"]),
    ...
    values = [p["varPct"] or 0 for p in series]
    fig, ax = plt.subplots(figsize=(6.2, 2.2))
    ax.plot(labels, values, color="#c94b2a", linewidth=2)
    ax.set_title("Tendencia YoY (%)", fontsize=9)
